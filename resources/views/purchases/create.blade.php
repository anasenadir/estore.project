...
@section('title')
{{ trans('purchases/create.page_title') }}
@stop
@section('css')
...
<div class="breadcrumb-header justify-content-between">
    <div class="left-content">
        <div>
            <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1">{{ trans('purchases/create.new_invoice_title') }}</h2>
        </div>
    </div>
</div>
<!-- row opened -->
    <form action='{{route('purchases.store')}}' method='POST' class="row">
        @csrf
                                    ...
                                    <select class="form-control select2" name='supplier_id'  data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" data-placeholder="chosse one" required="">
                                        ...
                                        @if (isset($suppliers))
                                            @foreach ($suppliers as  $supplier)
                                                <option @if (isset($request) &&$request->supplier_id == $supplier->id)
                                                    selected
                                                @endif
                                                value="{{$supplier->id}}">
                                                    {{$supplier->name}}
